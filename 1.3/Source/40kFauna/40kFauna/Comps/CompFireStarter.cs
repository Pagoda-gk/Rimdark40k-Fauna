        ...
        public override void CompTick()
        {
            base.CompTick();
            {
                ...
                if (pawn != null)
                {
                    ...
                    if (/*fireSize > 0.7f && */Rand.Value < fireSize * 0.01)
                    {
        ...
        protected void TrySpread()
        {
            ...
            if (Rand.Chance(FireUtility.ChanceToStartFireIn(intVec, pawn.Map)))
            {
                if (!flag)
                {
                    ...
                    ((Spark)GenSpawn.Spawn(ThingDefOf.Spark, pawn.Position, pawn.Map, WipeMode.Vanish)).Launch(pawn, intVec, intVec, ProjectileHitFlags.All);
                    return;
                }
                else
                {
